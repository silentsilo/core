...
| Index | `vault.db.enc` | `SCHEMA_VERSION = 5`, `silentsilo-vfs/schema.rs` | Not a format: see below |
...
copy's own table, `working_copy_state`, holds the BLAKE3 fingerprints that
decide that, and is dropped from every snapshot image. What `vault.db.enc`
seals is still a whole plain SQLite file image. A release before core 1.5.0 looks only for a plaintext `vault.db`,
