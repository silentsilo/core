...
/// Whether the user has set up Windows Hello: a face, a fingerprint or a PIN.
/// True on a machine with no TPM, where Hello holds its keys in software.
...
/// Whether this machine can enrol Windows Hello for a silo, which takes both
/// facts: Hello set up, and a TPM to derive from. Either one alone gets
/// somebody to the end of a ceremony that then fails.
pub(crate) fn platform_authenticator_available() -> bool {
    hello_enrolled() && tpm_present()
}

/// Whether this machine has a TPM at all.
///
/// Windows 11 is widely installed on older hardware with the TPM requirement
/// bypassed, and Hello still enrols there with its keys held in software. It
/// just has nothing to derive a PRF secret from, so enrolment runs the whole
/// ceremony and fails at the last step. Without this check the failure gets
/// blamed on the Windows build, which sends someone to buy a security key
/// over a Windows update that would not have helped.
///
/// TBS is the cheap answer and needs no elevation. A TPM disabled in firmware
/// reports the same as one that is absent, which is correct here: neither can
/// derive anything.
        ...
        return Err(FidoError::EnrollmentFailed(if via_phone {
            "This phone's passkey did not provide the hmac-secret extension the silo \
             ...
             or use Windows Hello."
                .into()
        } else if platform && !tpm_present() {
            // Windows 11 installed past its own hardware check is common, and
            // Hello enrols there quite happily with its keys in software. It
            // has nothing to derive a PRF secret from, which is this branch.
            // Blaming the Windows build here would send someone after an
            // update that cannot help.
            "This computer has no TPM, and Windows Hello derives the silo key from one. \
             Hello can still sign you in to Windows, but it cannot unlock a silo here. \
             Use a FIDO2 key instead (YubiKey 5, Nitrokey 3, SoloKeys)."
                .into()
        } else if platform {
