...
pub struct EncryptResult {
    ...
    pub size_bytes: u64,
    /// Plaintext bytes read, counted during encryption. What a file row
    /// records as the size, measured on the bytes actually sealed rather
    /// than re-read from a file that may have changed since.
    pub plain_bytes: u64,
}
...
pub fn encrypt_file(
    ...
    encrypt_stream(&mut source_file, dest, key, file_id, blob_id)
}

/// [`encrypt_file`] over any reader. Chunks are filled completely before
/// they are sealed: the decrypt side frames chunks by the fixed size, so a
/// short read mid-file would produce a blob that never decrypts again.
pub fn encrypt_stream<R: Read>(
    ...
    // Durable before returning: the record naming this blob can reach the
    // bucket within seconds, and a blob lost to a power cut under a record
    // every device has is a file the tree shows and nothing can ever open.
    dest_file.sync_all()?;
...
pub fn decrypt_blob(
    ...
    // Through a sibling temp file, renamed only once the hash holds: a
    // refused blob must not leave a shortened plaintext where the caller
    // asked for the file, nor destroy whatever was already at that path.
    let temp = {
        let mut name = dest.as_os_str().to_os_string();
        name.push(".part");
        std::path::PathBuf::from(name)
    };
    let mut dest_file = File::create(&temp)?;
    let key = Aes256Gcm::new_from_slice(key.as_bytes()).expect("valid key length");
    let result = decrypt_chunks(&mut source_file, &mut dest_file, &header, &key);
    // Dropped before either path, because Windows will neither rename nor
    // remove an open file.
    drop(dest_file);
    match result {
        Ok(()) => {
            silentsilo_core::rename_with_retry(&temp, dest)?;
            Ok(header)
        }
        Err(e) => {
...
/// The chunk loop of [`decrypt_blob`], including the hash comparison at the
/// end: everything that has to succeed before the plaintext may take the
/// caller's path.
fn decrypt_chunks(
    ...
    loop {
        ...
        let read = read_up_to(source_file, &mut chunk_buf)?;
        if read == 0 {
            break;
        }
        if read < TAG_SIZE {
...
fn read_up_to<R: Read>(source: &mut R, buf: &mut [u8]) -> Result<usize, CryptoError> {
    let mut filled = 0;
...
mod tests {
    ...
    /// A reader that answers in dribs, the way a network filesystem or an
    /// interrupted syscall can. Chunk framing must not depend on `read`
    /// filling the buffer.
    ...
    impl std::io::Read for DribblingReader {
        fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
            let n = self.most.min(buf.len()).min(self.data.len() - self.at);
    ...
    #[test]
    fn short_reads_do_not_shift_the_chunk_boundaries() {
        // Two chunks' worth through a reader that never fills the buffer.
        // Before `read_up_to` this produced undersized chunks mid-file, and
        // the decrypt side, which frames by the fixed chunk size, could
        // never open the blob again.
        let dir = tempdir().unwrap();
        let enc = dir.path().join("blob.sslo");
        ...
        let mut source = DribblingReader {
            data: data.clone(),
            at: 0,
            most: 8191,
        };
        let result = encrypt_stream(&mut source, &enc, &key, Uuid::new_v4(), blob_id).unwrap();

        assert_eq!(result.plain_bytes, data.len() as u64);
        // Framing check without decrypting: a full chunk, then the tail.
        let expected =
            HEADER_SIZE as u64 + (CHUNK_SIZE + TAG_SIZE) as u64 + (1234 + TAG_SIZE) as u64;
        assert_eq!(result.size_bytes, expected, "a chunk came out short");
    ...
    #[test]
    fn a_refused_decrypt_leaves_an_existing_destination_alone() {
        // The plaintext goes through a sibling temp file, so a blob that
        // fails its hash can neither shorten nor destroy a file already at
        // the path the caller chose.
        let dir = tempdir().unwrap();
