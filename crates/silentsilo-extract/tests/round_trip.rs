...
/// Password entries and their attachments are part of what a recovery has
/// to hand back: the tool once returned only the file tree, and the logins,
/// which the paper code exists to protect, stayed locked in the backup.
#[tokio::test]
async fn passwords_and_attachments_come_out_with_the_files() {
    let silo_dir = tempfile::tempdir().unwrap();
    let store_dir = tempfile::tempdir().unwrap();
    let out_dir = tempfile::tempdir().unwrap();
    let store = FolderStore::new(store_dir.path().to_path_buf());
    ...
    let vfs = Vfs::new(&session);
    let root = vfs.root_folder_id().unwrap();
    let file_blob = add_file(&session, root, "notes.txt", b"a file too");

    // An attachment blob, sealed like any content, referenced only from the
    // entry's JSON.
    let scratch = tempfile::tempdir().unwrap();
    let plain = scratch.path().join("scan.pdf");
    std::fs::write(&plain, b"attached bytes").unwrap();
    let attachment_blob = Uuid::new_v4();
    let content_key = generate_content_key();
    let wrapped = wrap_content_key(&content_key, &session.kek).unwrap();
    encrypt_file(
        &plain,
        &session.paths.blob_path(attachment_blob),
        &content_key,
        Uuid::now_v7(),
        attachment_blob,
    )
    ...
    vfs.upsert_password(
        Uuid::new_v4(),
        &format!(
            r#"{{"id":"e1","service":"example.com","username":"alex","password":"hunter2","url":"https://example.com","notes":"","category":"work","attachments":[{{"blob_id":"{attachment_blob}","name":"scan.pdf","size_bytes":14,"blob_key":"{wrapped}"}}]}}"#
        ),
    )
    .unwrap();
    ...
    let backup = silentsilo_extract::open(&store as &dyn ObjectStore, &code)
        .await
        .unwrap();
    assert_eq!(backup.password_count(), 1);
    ...
    let csv =
        std::fs::read_to_string(out_dir.path().join("_passwords").join("passwords.csv")).unwrap();
