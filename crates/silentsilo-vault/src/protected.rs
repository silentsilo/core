...
fn config_path(vault_root: &Path) -> PathBuf {
    crate::workdir::cache_dir_for(vault_root).join("protected.json")
}
...
pub fn save_protected(vault_root: &Path, folders: &ProtectedFolders) -> Result<(), VaultError> {
    crate::workdir::write_private(&config_path(vault_root), &crate::format::encode(folders)?)?;
    Ok(())
}
...
mod tests {
    ...
    fn the_folder_list_never_lands_in_the_silo() {
        ...
        let back = load_protected(&silo).unwrap();
