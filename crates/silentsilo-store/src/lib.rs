...
pub trait ObjectStore: Send + Sync {
    ...
    /// Reads an object into a file on disk, replacing whatever is there.
    /// Same reasoning as [`ObjectStore::put_from_file`].
...
mod file_transfer_tests {
    ...
    /// The smallest possible backend, to exercise the trait's default file
    /// methods rather than any override.
    struct MemoryStore(Mutex<HashMap<String, Vec<u8>>>);

    #[async_trait]
    impl ObjectStore for MemoryStore {
        async fn put(&self, key: &str, body: Vec<u8>) -> Result<(), StoreError> {
            self.0.lock().unwrap().insert(key.into(), body);
            Ok(())
        }
        ...
        async fn head(&self, key: &str) -> Result<Option<i64>, StoreError> {
            Ok(self.0.lock().unwrap().get(key).map(|b| b.len() as i64))
        }
    ...
    #[tokio::test]
    async fn the_default_file_methods_round_trip() {
        ...
        store.put_from_file("blobs/a.sslo", &source).await.unwrap();
        store.get_to_file("blobs/a.sslo", &restored).await.unwrap();

        assert_eq!(std::fs::read(&restored).unwrap(), b"sealed bytes");
    }

    #[tokio::test]
    async fn the_folder_store_streams_a_file_through_disk() {
        let target = tempfile::tempdir().unwrap();
        let store = FolderStore::new(target.path().to_path_buf());
        let dir = tempfile::tempdir().unwrap();
        let source = dir.path().join("in.bin");
        let restored = dir.path().join("out.bin");
        std::fs::write(&source, vec![7u8; 100_000]).unwrap();

        store.put_from_file("blobs/b.sslo", &source).await.unwrap();

        // No `.part` left behind, and the object lists at its real size.
        let listed = store.list("blobs/").await.unwrap();
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].size, 100_000);

        store.get_to_file("blobs/b.sslo", &restored).await.unwrap();
        assert_eq!(std::fs::read(&restored).unwrap(), vec![7u8; 100_000]);

        let missing = store.get_to_file("blobs/absent.sslo", &restored).await;
