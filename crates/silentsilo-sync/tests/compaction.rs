...
use silentsilo_sync::{
    JoinPlan, SyncError, apply_rebuild, fetch_join_plan, fetch_rebuild, rebootstrap_from_snapshot,
    sync_ops,
};
...
async fn a_device_joining_a_compacted_silo_sees_the_whole_tree() {
    ...
    let whole_log = silentsilo_sync::fetch_all_ops_above(&store, &dek, 0)
        .await
        .unwrap();
...
#[tokio::test]
async fn every_way_into_a_compacted_silo_rebuilds_the_whole_tree() {
    // Both joins, the one that unwraps a key envelope and the one that
    // unwraps a recovery code, go through `fetch_join_plan`. They used to
    // decide separately, and the recovery path decided wrong: it replayed
    // the log from zero, which after a compaction is only what sits above
    // the horizon. That handed somebody rebuilding from paper a vault
    // missing everything older than the last snapshot, with nothing on
    // screen to say so, in the one situation where there is no other copy
    // left to notice it against.
    let (_dir, store, dek, awake, _asleep, horizon) =
        silo_compacted_behind_a_sleeping_device().await;

    let plan = fetch_join_plan(&store, &dek).await.unwrap();
    assert!(
        matches!(plan, JoinPlan::FromSnapshot { ref snapshot, .. } if snapshot.horizon == horizon),
        "a compacted silo must be joined from its snapshot"
    );

    let mut newcomer = Device::joining(awake.session.vault_id);
    plan.apply(&mut newcomer.session.conn).unwrap();
...
#[tokio::test]
async fn an_uncompacted_silo_is_joined_by_replaying_its_whole_log() {
    // The other half of the same decision, and the reason the plan is read
    // from storage rather than assumed: most silos have never been
    // compacted, and refusing to join one that has no snapshot would be a
    // worse bug than the one this replaced.
    let (_dir, store) = store();
    let dek = generate_dek();
    let device = Uuid::new_v4();
    push_ops(&store, &dek, &[record(1, device), record(2, device)])
        .await
        .unwrap();

    let plan = fetch_join_plan(&store, &dek).await.unwrap();
    assert!(matches!(plan, JoinPlan::WholeLog(_)));
...
#[tokio::test]
async fn a_blob_comes_from_whichever_copy_holds_it() {
    // The first target being an unplugged disk, or simply behind, must not
    // fail a download the second copy can serve.
    let (_dir_a, empty) = store();
    let (_dir_b, holds_it) = store();
    let vault = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(vault.path().join("blobs")).unwrap();
    let blob = Uuid::new_v4();
    put_blob(&holds_it, blob).await;

    let stores: Vec<&dyn ObjectStore> = vec![&empty, &holds_it];
    let size = silentsilo_sync::fetch_blob_from_any(&stores, vault.path(), blob)
        .await
        .unwrap();

    assert_eq!(size, b"ciphertext".len() as u64);
    assert!(
        vault
            .path()
            .join("blobs")
...
async fn purged_content_another_device_still_references_survives_the_sweeps() {
    ...
    // object at purge time froze "referenced" at what one device knew and
    // left B's row pointing at nothing; the sweep re-asks after the ops have
    // converged.
    let (_dir, store) = store();
    let dek = generate_dek();
    ...
    put_blob(&store, shared).await;

    let f1 = Vfs::new(&a.session)
        .add_file(a.root(), "one.txt", shared, 3, "h", None, "k")
        .unwrap();
    a.sync(&store, &dek).await.unwrap();
    b.sync(&store, &dek).await.unwrap();

    // B references the same content, offline.
    Vfs::new(&b.session)
        .add_file(b.root(), "two.txt", shared, 3, "h", None, "k")
        .unwrap();

    // A empties its trash, not knowing about B's row.
    Vfs::new(&a.session).trash_file(f1.id).unwrap();
    let (_, orphaned) = Vfs::new(&a.session).empty_trash().unwrap();
    assert_eq!(orphaned, vec![shared], "locally the blob looks orphaned");

    a.sync(&store, &dek).await.unwrap();
    b.sync(&store, &dek).await.unwrap();
    a.sync(&store, &dek).await.unwrap();

    // Two sweep passes from A, after convergence: B's row keeps the blob.
    let referenced = Vfs::new(&a.session)
        .referenced_blobs_with_attachments()
        .unwrap();
    let first = sweep_orphan_blobs(&store, &referenced, &HashSet::new())
        .await
        .unwrap();
    let second = sweep_orphan_blobs(&store, &referenced, &first.candidates.into_iter().collect())
        .await
        .unwrap();

    assert_eq!(second.deleted, 0);
    assert_eq!(
        blobs_left(&store).await,
        1,
        "the shared content was deleted"
    );
}

#[tokio::test]
async fn a_password_attachment_survives_the_sweep() {
    // An attachment has no row in `files`: the sealed entry is its only
    // reference. The sweep once took its referenced set from the tree alone,
    // so every attachment older than two passes was deleted from the bucket.
    let (_dir, store) = store();
    let device = Device::joining(Uuid::new_v4());
    let attachment = Uuid::new_v4();
    put_blob(&store, attachment).await;

    let vfs = Vfs::new(&device.session);
    vfs.upsert_password(
        Uuid::new_v4(),
        &format!(
            r#"{{"id":"x","attachments":[{{"blob_id":"{attachment}","name":"scan.pdf","size_bytes":9,"blob_key":"wrapped"}}]}}"#
        ),
    )
    .unwrap();
    ...
    assert!(
        referenced.contains(&attachment),
        "the attachment is not in the referenced set"
    );

    // Two passes, the second of which deletes what stayed unreferenced.
    let first = sweep_orphan_blobs(&store, &referenced, &HashSet::new())
        .await
        .unwrap();
    let second = sweep_orphan_blobs(&store, &referenced, &first.candidates.into_iter().collect())
    ...
    assert_eq!(second.deleted, 0);
    assert_eq!(blobs_left(&store).await, 1, "the attachment was swept away");
}
...
async fn fetching_reports_a_total_before_the_first_record_arrives() {
    ...
    let fetched =
        silentsilo_sync::fetch_all_ops_above_reporting(&store, &dek, 0, &mut |done, total| {
...
/// A store that quietly corrupts snapshot writes, the way a broken provider
/// or a flipped bit in transit would.
struct CorruptingStore(FolderStore);

#[async_trait::async_trait]
impl ObjectStore for CorruptingStore {
    async fn put(&self, key: &str, body: Vec<u8>) -> Result<(), silentsilo_store::StoreError> {
        let body = if key.starts_with("snapshots/") {
            b"rotted in transit".to_vec()
        } else {
            body
        };
...
#[tokio::test]
async fn a_snapshot_that_does_not_read_back_prunes_nothing() {
    // The snapshot is about to become the only copy of everything below the
    // horizon. If the provider corrupts the write, the records must stay:
    // trading them for an unreadable replacement is how a silo becomes
    // unjoinable with nothing said.
    let dir = tempfile::tempdir().unwrap();
    let store = CorruptingStore(FolderStore::new(dir.path().to_path_buf()));
    let dek = generate_dek();
    let device = Uuid::new_v4();
    let records: Vec<OpRecord> = (1..=5).map(|lamport| record(lamport, device)).collect();
    push_ops(&store, &dek, &records).await.unwrap();

    let snapshot = snapshot_at(Uuid::new_v4(), 3);
    let err = silentsilo_sync::publish_compaction(&store, &dek, &snapshot, true).await;

    assert!(err.is_err(), "a corrupt snapshot still pruned");
    assert_eq!(
        store.list("ops/").await.unwrap().len(),
        5,
