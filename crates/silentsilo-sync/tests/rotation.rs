...
use silentsilo_sync::{
    CONTENT_KEK_KEY, OPS_PREFIX, fetch_all_ops_above, publish_content_kek, push_ops, put_snapshot,
...
async fn after_rotation_the_old_key_opens_nothing_and_the_new_one_opens_everything() {
    ...
    assert!(
        fetch_all_ops_above(&store as &dyn ObjectStore, &old, 0)
            .await
            .is_err(),
        "the old vault key must no longer open the log"
    );
    let after = fetch_all_ops_above(&store as &dyn ObjectStore, &new, 0)
        .await
        .unwrap();
...
#[tokio::test]
async fn a_device_the_key_moved_on_from_is_told_before_it_pushes() {
    // The published KEK envelope is sealed under the current vault key, so
    // it doubles as the rotation detector: a device whose key no longer
    // opens it was rotated away and must not push.
    let dir = tempfile::tempdir().unwrap();
    let store = FolderStore::new(dir.path().to_path_buf());
    let kept = generate_dek();
    let rotated_away = generate_dek();

    // Nothing published yet: nothing to compare against.
    assert_eq!(
        silentsilo_sync::key_still_current(&store as &dyn ObjectStore, &kept)
            .await
            .unwrap(),
        None
    );
    ...
    let kek = generate_content_kek();
    let envelope = seal(kek.as_bytes(), &kept).unwrap();
    publish_content_kek(&store as &dyn ObjectStore, &envelope)
        .await
        .unwrap();
    ...
    assert_eq!(
        silentsilo_sync::key_still_current(&store as &dyn ObjectStore, &rotated_away)
            .await
            .unwrap(),
        Some(false),
